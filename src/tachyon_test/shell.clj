...
(defn fork
  "Forks off a command, returning a java Process. If :inheritIO is true, :out and :err will be 
  empty."
...
(defn wait-for
  "Waits for a given Process to complete. If it doesn't complete within the given timeout period, 
  then :status will be :timeout and process is killed"
  ...
  (try
    (let [rc (.get (future-call #(.waitFor p)) timeout time-unit)]
      {:status :ok 
       :exit rc 
       :out (copy-str (.getInputStream p)) 
       :err (copy-str (.getErrorStream p))})
