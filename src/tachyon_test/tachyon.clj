...
(defn create-file
  "Attempts to create a file at the given path. If the path already exists or any other exception 
  happens, a string error message is returned"
  [fs path]
  (let [fid (.createFile fs path)]
    (.getFile fs fid)))

(defn is-pred
  "Runs the is function against the given predicate. If fails the message is generated by the m 
  function"
  [f m]
  (fn [a]
    (do
